      ...
      @foreach(\App\Models\Service::where('is_active', true)->with('images')->limit(3)->get() as $service)
        <div class="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col">
          ...
          <div class="relative h-64 rounded-t-lg overflow-hidden flex-shrink-0">
            @if($service->images->isNotEmpty())
              <div class="image-slider-container relative w-full h-full">
                @foreach($service->images as $index => $image)
                  <div class="image-slide absolute inset-0 transition-opacity duration-500 ease-in-out {{ $index === 0 ? 'opacity-100' : 'opacity-0' }}">
                ...
                @if($service->images->count() > 1)
                  <button class="slider-prev absolute left-0 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-30 hover:bg-opacity-50 text-white rounded-r-full p-3 transition">
                    <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                    </svg>
                  </button>
                  <button class="slider-next absolute right-0 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-30 hover:bg-opacity-50 text-white rounded-l-full p-3 transition">
                    <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            @else
              <img src="https://i.ibb.co/2c8Q0kM/default-avatar.png" alt="{{ $service->name }}" class="w-full h-full object-cover">
            @endif
          </div>
...
<div class="py-24 bg-white">
  <div class="max-w-7xl mx-auto px-4 lg:px-8">
    <div class="text-center max-w-3xl mx-auto mb-20">
      <h2 class="text-base font-semibold text-primary-600 uppercase">{{ __('website.why_choose_us') }}</h2>
      <h3 class="mt-2 text-3xl font-extrabold text-dark-900">{{ __('website.everything_you_need') }}</h3>
      <p class="mt-4 text-xl text-dark-500">{{ __('website.discover_our_benefits') }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
      ...
      <div class="text-center">
        ...
        <h4 class="mt-6 text-lg font-bold text-dark-900">{{ __('website.fast_booking') }}</h4>
        <p class="mt-3 text-base text-dark-500">{{ __('website.book_instantly') }}</p>
      </div>
      ...
      <div class="text-center">
        ...
        <h4 class="mt-6 text-lg font-bold text-dark-900">{{ __('website.secure_payments') }}</h4>
        <p class="mt-3 text-base text-dark-500">{{ __('website.safe_transaction') }}</p>
      </div>
      ...
      <div class="text-center">
        ...
        <h4 class="mt-6 text-lg font-bold text-dark-900">{{ __('website.reminders') }}</h4>
        <p class="mt-3 text-base text-dark-500">{{ __('website.never_miss_appointment') }}</p>
      </div>
      ...
      <div class="text-center">
        ...
        <h4 class="mt-6 text-lg font-bold text-dark-900">{{ __('website.expert_professionals') }}</h4>
        <p class="mt-3 text-base text-dark-500">{{ __('website.quality_service') }}</p>
      </div>
  ...
  <div class="max-w-7xl mx-auto px-4 lg:px-8">
    <div class="grid grid-cols-2 md:grid-cols-4 gap-12 text-center text-white">
      <div class="p-6">
        ...
        <div class="mt-3 text-lg font-medium">{{ __('website.happy_customers') }}</div>
      </div>
      <div class="p-6">
        <div class="text-4xl font-extrabold">500+</div>
        <div class="mt-3 text-lg font-medium">{{ __('website.services_offered') }}</div>
      </div>
      <div class="p-6">
        <div class="text-4xl font-extrabold">99%</div>
        <div class="mt-3 text-lg font-medium">{{ __('website.customer_satisfaction') }}</div>
      </div>
      <div class="p-6">
        <div class="text-4xl font-extrabold">24/7</div>
        <div class="mt-3 text-lg font-medium">{{ __('website.support_available') }}</div>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="max-w-7xl mx-auto px-4 lg:px-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      <div class="bg-white rounded-lg shadow-lg p-8">
        <div class="flex items-center mb-6">
          <div class="flex-shrink-0">
            <img class="h-16 w-16 rounded-full" src="{{ asset('images/testimonial-1.jpg') }}" alt="Testimonial 1">
          </div>
          <div class="ml-4">
            <h4 class="text-lg font-bold text-dark-900">John Doe</h4>
            <p class="text-primary-600">{{ __('website.customer') }}</p>
          </div>
        </div>
      ...
      <div class="bg-white rounded-lg shadow-lg p-8">
        <div class="flex items-center mb-6">
          <div class="flex-shrink-0">
            <img class="h-16 w-16 rounded-full" src="{{ asset('images/testimonial-2.jpg') }}" alt="Testimonial 2">
          </div>
          <div class="ml-4">
