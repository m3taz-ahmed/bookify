        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl">
            <div class="px-6 py-6 border-b border-gray-200 bg-gradient-to-r from-background-50 to-accent-50 relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <h1 class="text-3xl font-bold text-gray-900 heading-gradient">{{ $page->title }}</h1>
                </div>
            </div>
                ...
                @if($page->type === \App\Models\Page::TYPE_ABOUT_US)
                    <div class="prose prose-lg max-w-none fade-in">
                        @if($page->company_name_en || $page->company_name_ar)
                            <h2 class="text-2xl font-bold text-primary-800 mt-2">{{ app()->getLocale() === 'ar' ? $page->company_name_ar : $page->company_name_en }}</h2>
                        @endif
                        
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                            @if($page->founded_year)
                                <div class="bg-background-50 p-4 rounded-lg border border-background-200">
                                    <h3 class="font-semibold text-primary-700">{{ __('website.founded') }}</h3>
                                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $page->founded_year }}</p>
                                </div>
                            @endif
                            
                            @if($page->location_en || $page->location_ar)
                                <div class="bg-background-50 p-4 rounded-lg border border-background-200">
                                    <h3 class="font-semibold text-primary-700">{{ __('website.location') }}</h3>
                                    <p class="text-gray-900 mt-1">{{ app()->getLocale() === 'ar' ? $page->location_ar : $page->location_en }}</p>
                                </div>
                            @endif
                            
                            @if($page->company_description_en || $page->company_description_ar)
                                <div class="bg-background-50 p-4 rounded-lg border border-background-200">
                                    <h3 class="font-semibold text-primary-700">{{ __('website.company') }}</h3>
                                    <p class="text-gray-900 mt-1 line-clamp-3">{{ strip_tags(app()->getLocale() === 'ar' ? $page->company_description_ar : $page->company_description_en) }}</p>
                                </div>
                            @endif
                        </div>
                        ...
                        @if($page->history_en || $page->history_ar)
                            <div class="mt-8">
                                <h3 class="text-xl font-bold text-primary-800 mb-3">{{ __('website.history') }}</h3>
                                <div class="prose max-w-none">
