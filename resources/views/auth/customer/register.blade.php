...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-background-50 to-background-100 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
        <div class="bg-white rounded-2xl shadow-xl p-8 transition-all duration-300 hover:shadow-2xl border-t-4 border-primary">
            ...
            <form class="mt-6 space-y-6" method="POST" action="{{ route('customer.register', ['locale' => $currentLocale]) }}">
                @csrf
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input id="phone" name="phone" type="text" class="w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('phone') border-red-500 @enderror transition duration-200" placeholder="{{ __('website.phone_placeholder') }}" value="{{ old('phone') }}" autocomplete="tel">
                        </div>
                        @error('phone')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input id="password" name="password" type="password" required class="w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('password') border-red-500 @enderror transition duration-200" placeholder="{{ __('website.password_placeholder') }}" autocomplete="new-password">
                        </div>
                        @error('password')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        <label for="password-confirm" class="block text-sm font-medium text-gray-700 mb-2">{{ __('website.confirm_password') }}</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <svg class="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                    <path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd" />
                            ...
                            <input id="password-confirm" name="password_confirmation" type="password" required class="w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition duration-200" placeholder="{{ __('website.confirm_password_placeholder') }}" autocomplete="new-password">
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-6 text-center">
                <p class="text-sm text-gray-600">{{ __('website.already_have_account') }} 
                    <a href="{{ route('customer.login', ['locale' => $currentLocale]) }}" class="font-medium text-primary-600 hover:text-primary-500 transition-colors duration-200">{{ __('website.sign_in') }}</a>
                </p>
            </div>
        </div>
