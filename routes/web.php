...
Route::get('/', function (\Illuminate\Http\Request $request) {
    // Set default locale in session if not already set
    if (!$request->hasSession() || !$request->session()->isStarted()) {
        // Force session start for this request
    ...
    if (!$request->session()->has('locale')) {
        ...
        // Log that we've set the locale
        \Illuminate\Support\Facades\Log::info('Setting default locale in welcome route', [
            'session_locale_after_set' => $request->session()->get('locale')
        ]);
    }
